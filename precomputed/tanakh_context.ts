...
class Gemini extends PromptRequester {
...
export async function llmMain(): Promise<void> {
  ...
  const model = new LanguageModel(new Gemini(modelConfiguration));
  ...
  await parallelizeRun(
    async function runLlm(chapter) { // eslint-disable-line prefer-arrow-callback
      ...
      const prompt = [
        ...
        "\n",
        "When multiple verses are part of the same topic, prefer to use the same heading.",
  ...
  writeJson("precomputed/tanakh_contexts_gemini_1.5_flash.json", result);
}
