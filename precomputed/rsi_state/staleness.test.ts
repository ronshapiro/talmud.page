...
test("checkTextStaleness Tier 0: cosmetic-only change (punctuation/whitespace) is fresh", () => {
  ...
  expect(result).toEqual({status: StalenessStatus.Fresh, tier: StalenessTier.Fingerprint});
});
...
test("checkTextStaleness Tier 1: small real edit under the cosmetic threshold is fresh", () => {
  ...
  expect(result.tier).toBe(StalenessTier.EditDistance);
  expect(result.status).toBe(StalenessStatus.Fresh);
...
test("checkTextStaleness Tier 2: mid-range edit needs classification", () => {
  ...
  const result = checkTextStaleness(stored, current, DEFAULT_STALENESS_THRESHOLDS);
  expect(result.status).toBe(StalenessStatus.NeedsClassification);
  expect(result.tier).toBe(StalenessTier.AgenticClassification);
...
test("checkTextStaleness Tier 3: large rewrite is stale without classification", () => {
  ...
  expect(result.status).toBe(StalenessStatus.Stale);
  expect(result.tier).toBe(StalenessTier.StructuralBreak);
});
...
test("resolveWithClassification marks !stillValid as stale at tier 2", () => {
  ...
  expect(resolved.status).toBe(StalenessStatus.Stale);
});
...
test("checkSegmentCountStaleness: unchanged count is fresh", () => {
  expect(checkSegmentCountStaleness(5, 5))
    .toEqual({status: StalenessStatus.Fresh, tier: StalenessTier.Fingerprint});
});
