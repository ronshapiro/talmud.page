...
export async function sendEmail({to, cc, subject, text, html}: SendParameters): Promise<unknown> {
  ...
  const ignoreAddresses = fs.existsSync("ignore_email_addresses")
    ? new Set(readUtf8("ignore_email_addresses").split(","))
    : new Set<string>();
      ...
      Messages: [
        {
          ...
          To: [makeEmail(to)],
          CC: (cc ?? []).filter(x => !ignoreAddresses.has(x)).map(x => makeEmail(x)),
          Subject: subject,
          TextPart: text,
          HTMLPart: html,
        },
