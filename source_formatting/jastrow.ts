...
interface AbbreviationBeforeCompilation {
  abbreviation: string;
  expanded: string;
  exceptions?: string[];
}

interface Abbreviation extends AbbreviationBeforeCompilation {
  regex: RegExp;
}

const _ABBREVIATIONS: AbbreviationBeforeCompilation[] = [
  {
    abbreviation: "a.",
    expanded: "and",
  },
...
const ABBREVIATIONS: Abbreviation[] = _ABBREVIATIONS.map(x => {
  return {
    ...x,
    regex: new RegExp("(^| |—)(\\()?" + x.abbreviation.replace(/\./g, "\\."), "g"),
  };
});
...
function deabbreviate(text: string): string {
  for (const abbreviation of ORDERED_ABBREVIATIONS) {
    text = applyRegex(text, abbreviation);
  }
  return text;
}
...
export class JastrowReformatter extends HtmlVisitor {
  ...
  visitText(text: string): void {
    ...
    this.appendText(deabbreviate(text));
  }
}
